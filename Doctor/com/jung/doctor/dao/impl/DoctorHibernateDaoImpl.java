...
public class DoctorHibernateDaoImpl extends HibernateEntityManagerImpl<Doctor>
	...
	@Override
	public List<Doctor> getDoctors(String hql) {
		// TODO Auto-generated method stub
	...
	public int getDoctorPointsByID(int doctorID){
		String sql = "select doctorPoints from Doctor where doctorID="+doctorID;
	try {
		List<Object[]> list = super.executeSQL(sql);
		if (list != null && list.size() > 0) {
			return Integer.parseInt(String.valueOf(list.get(0)));
	...
	public String activeDoctor(int doctorID, String doctorActivateOption) {
		...
		String sql="update doctor set doctorActivateOption='"+doctorActivateOption+"'"+" ,password='"+password+"'"+" where doctorID="+doctorID;
		try {
			super.executeSQLUpdate(sql);
		} catch (SkeletonException e) {
			// TODO Auto-generated catch block
	...
	public boolean deleteDoctorByRegionID(int regionID){
		String sql="delete from Doctor where hospitalID="+regionID;
		try {
			super.executeSQLUpdate(sql);
			return true;
		} catch (SkeletonException e) {
   ...
   public Doctor getDoctorByRegionID(int regionID){
	   String hql="from Doctor as Doctor where doctor.region.regionID="+regionID;
	   try {
		List doctorList=super.executeHql(hql);
		if(doctorList!=null&&doctorList.size()>0){
			return (Doctor) doctorList.get(0);

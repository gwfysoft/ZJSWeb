...
public class EmployeeHibernateDaoImpl  extends HibernateEntityManagerImpl<Employee> implements EmployeeHibernateDao{
	...
	private static final Log logger = LogFactory.getLog(EmployeeHibernateDaoImpl.class);
	...
	public boolean updatePassword(int employeeID, int newPassword) {
      ...
      public  List<Employee> getEmployeeByRegionID(int regionID){
    	  String hql="from Employee as employee where employee.region.regionID="+regionID;
			try {
				List employeeList =super.executeHql(hql);
				if(employeeList!=null&&employeeList.size()>0){
					return employeeList;
	...
	/**
	 * @param doctorHibernateDao the doctorHibernateDao to set
	 */
     @Resource

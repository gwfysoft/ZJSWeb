...
//	private UserService userService;
	...
	public String modifyRoleOwnedMenus() {
		...
		List<RoleForUserLinkMenu> listRCM = userCompetenceService
			...
			userCompetenceService.updateRoleForUser(roleName, Integer.parseInt(roleID));
	...
	public String displayUserOwnedRoles() {
		...
		if (roleForUsers != null) {
			Employee employee = employeeService.getEmployeeById(Integer.parseInt(userID));
			if (employee != null) {
				roleName = employee.getEmployeeName();// 此处roleName代表userName
			}
			return SUCCESS;
		} else {
			return FAIL;
	...
	public String assignRoleForUser() {
		if (!roleName.equals("")) {
			...
			for (String roleID : roleIDS) {
				...
				rlc.setUserID(Integer.parseInt(userID));
...
//				clm.setCompanyID(CompetenceUtil.getCompany().getCompanyID());
	...
	public String displayModifyUserOwnedRoles() {
		...
		userOwnedRoles = userCompetenceService.getRCCByUser(Integer.parseInt(userID));// 根据用户ID获取该用户的所有已绑定的角色
